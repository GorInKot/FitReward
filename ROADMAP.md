...
- **Названия упражнений на английском** — 🟡 частично: скрипт `translate:exercises` с курируемым словарём ~90 упражнений проставляет `nameRu`. Покрывает частые движения; редкие остаются англоязычными. Расширять словарь по списку untranslated из вывода скрипта.

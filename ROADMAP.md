...
## Фаза 13. Polish / DX (P3) — 🟡 КОД ГОТОВ, ОЖИДАЕТ ПРОДА
...
### Тест
- [ ] Прод: переключить тему Telegram (светлая/тёмная) при режиме «Авто» — приложение следует за ней
- [ ] Профиль → переключатель темы Авто/Светлая/Тёмная работает и сохраняется между запусками
- [ ] Loading skeletons видны при заходе на экраны до загрузки данных
- [ ] Ошибка API (напр. оффлайн) показывается тостом сверху
...
## Фаза 15. Инструкции к упражнениям (P2)

Если пользователь не знает, как выполняется упражнение (напр. «Жим Арнольда») — показать технику прямо в приложении.

- [ ] Контент: описание техники, типичные ошибки, задействованные мышцы (поле `instructions` в `ExerciseCatalog` / `curatedExercises` уже есть — проверить заполнение и качество для ~85 курируемых упражнений)
- [ ] Визуал: изображение / гиф / видео выполнения (`imageUrl` в схеме есть; выбрать источник медиа)
- [ ] UI: тап по упражнению в «Планах» и «Тренировке» → детальная карточка/модалка с техникой
- [ ] Двуязычные инструкции RU/EN

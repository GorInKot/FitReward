...
## Фаза 11. Бот в проде + напоминания (P2) — ✅ ВЫПОЛНЕНО (2026-05-16)
...
- [x] Render: создать Web Service `fitreward-bot`, env-переменные
- [x] @BotFather: `/setmenubutton` → Mini App URL, `/setcommands`
- [x] cron-job.org: ежечасный `POST .../tasks/reminders` с `X-Internal-Secret`
- [x] Прод-тест в Telegram: команды + напоминание
...
## Фаза 13. Polish / DX (P3) — ✅ ВЫПОЛНЕНО (2026-05-16)
...
- [x] Loading skeletons — `components/Skeleton.tsx`, заменили текстовые «Загрузка...» на Home/Workout/Plans/Progress
- [x] Тост-нотификации для ошибок API — `store/toastStore.ts` + `components/Toast.tsx`, инлайновые ошибки переведены на тосты
- [x] Адаптивная тема (светлая/тёмная):
  - Семантические токены через CSS-переменные (`surface`/`panel`/`elevated`/`hairline`/`ink`) в `tailwind.config.js` + `index.css`, `darkMode: 'class'`
  - `store/themeStore.ts`: режим Авто/Светлая/Тёмная; «Авто» берёт `WebApp.colorScheme` + слушает `themeChanged`; выбор в localStorage; красит хедер Telegram
  - Переключатель темы в Профиле
- [x] README — реальные инструкции по монорепо, локальному запуску, env-переменным и деплою

### Тест
- [x] Прод: переключить тему Telegram (светлая/тёмная) при режиме «Авто» — приложение следует за ней
- [x] Профиль → переключатель темы Авто/Светлая/Тёмная работает и сохраняется между запусками
- [x] Loading skeletons видны при заходе на экраны до загрузки данных
- [x] Ошибка API (напр. оффлайн) показывается тостом сверху

---

## Фаза 14. Гид по приложению (онбординг-тур) (P2) — ✅ ВЫПОЛНЕНО (2026-05-16)

При первом запуске — обзор разделов приложения, чтобы пользователь сразу понимал, что где находится и как этим пользоваться.

- [x] Формат: пошаговые экраны-карточки (полноэкранный оверлей `components/AppGuide.tsx`)
- [x] Контент по разделам: вступление + Главная, Тренировка, Планы, Прогресс, Профиль
- [x] Показывать гид один раз — флаг `fitreward.guideSeen` в localStorage (`store/guideStore.ts`); авто-открытие после онбординга на первом реальном экране
- [x] Кнопка «Открыть гид» в Профиле для повторного запуска
- [x] Переводы RU/EN (блок `guide`)

### Тест
- [x] Прод: первый вход после онбординга → появляется гид, листается, закрывается
- [x] Кнопка «Открыть гид» в Профиле открывает его снова
- [x] Закрыть/открыть Mini App → гид больше не появляется автоматически

---

## Фаза 15. Инструкции к упражнениям (P2) — 🟡 КОД ГОТОВ, ОЖИДАЕТ ПРОДА

Тап «?» по упражнению в «Планах» и «Тренировке» → модалка с техникой, ошибками, мышцами и фото.

- [x] Контент: двуязычный гид `frontend/src/data/exerciseGuide.ts` — техника + типичные ошибки для всех 83 упражнений; лениво загружается отдельным чанком (~14 КБ gzip, не в стартовом бандле)
- [x] Визуал: 73 фото из free-exercise-db (public domain) в `frontend/public/exercises/`; скрипт `frontend/scripts/fetch-exercise-images.mjs` сопоставил каталог с датасетом; 10 упражнений (резинка/нишевые) без фото — graceful через `onError`
- [x] UI: `components/ExerciseGuideModal.tsx` (bottom-sheet) + `store/exerciseGuideStore.ts`; кнопка «?» в «Планах» и «Тренировке»
- [x] Двуязычные инструкции RU/EN + локализованные названия мышц (`muscle.*`)

### Тест
- [ ] Прод: тап «?» на упражнении в Планах и Тренировке → модалка с фото/техникой/ошибками
- [ ] Переключение языка → контент модалки меняется
- [ ] Упражнение без фото (напр. `band-curl`) → модалка без картинки, без битой иконки

---

## Фаза 16. Платежи, подписка и реферальная программа (P3) — ⏸ ОТЛОЖЕНО

> Разбор 2026-05-17: внутри Telegram Mini App цифровая подписка оплачивается **только через Telegram Stars** — карта/крипта за неё нарушают ToS (риск бана). Реферальные отчисления — это встроенная **Affiliate Program** Telegram (комиссия в Stars, начисляет сам Telegram), а не наш код. Разделы ниже — исходный план, требует переписи под Stars. Сначала — усиление premium-ценности (Фаза 9, AI-коуч).

Расширяет Фазу 12 (Telegram Stars) до полноценной монетизации с внешними платежами и рефералкой.

### Способы оплаты
- [ ] Банковская карта (выбрать провайдера-эквайринга)
- [ ] Криптовалюта (выбрать платёжный шлюз)
- [ ] Telegram Stars как один из вариантов (см. Фаза 12)

### Подписка
- [ ] Механизм подписки с регулярной оплатой (тарифы, срок, продление)
- [ ] Финализировать расширенный платный функционал (premium) — кандидаты: безлимитный AI-коуч, продвинутая аналитика, эксклюзивные программы
- [ ] Гейтинг premium-фич в UI, бейдж Premium

### Реферальная программа
- [ ] Реферальные ссылки — уникальная ссылка-приглашение на каждого пользователя
- [ ] Учёт, кто кого пригласил (`User.referredBy` / отдельная таблица рефералов)
- [ ] Баланс пользователя (руб / крипта / USD-эквивалент)
- [ ] Отчисления: когда приглашённые суммарно оплатили 100·N (руб или эквивалент в крипте/USD), пригласивший получает N на баланс
- [ ] Условия начисления и анти-фрод (защита от накруток рефералов)
- [ ] Использование баланса — решить (оплата подписки с баланса? вывод средств?)
...
### 2026-05-17
- [x] Нижняя навигация поднята над зоной жестов (safe-area inset, `viewport-fit=cover`) + иконки разделов (`components/NavIcon.tsx`)
- [x] Выбор дней тренировок: поле `User.trainingDays` (миграция `20260517_training_days`), пикер дней недели в Профиле; пустой массив → дефолт по частоте; бот-напоминания и `/internal` используют выбранные дни
